...
public class new_1234259{
    ...
    public void testChangeText_sameActivity() throws InterruptedException, RemoteException {
        ...
        target.click();
        target = mDevice.wait(Until.findObject(By.text("Add Podcast").res("de.danoeh.antennapod.debug:id/txtvTitle")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.textContains("Add Podcast by")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.clazz("android.widget.EditText")),2000);
        target.setText("https://feeds.simplecast.com/54nAGcIl");
        target = mDevice.wait(Until.findObject(By.text("Confirm")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("Subscribe")),5000);
        target.click();
        TimeUnit.MILLISECONDS.sleep(4000);
        target = mDevice.wait(Until.findObject(By.descContains("Search")),5000);
        target.click();
        target = mDevice.wait(Until.findObject(By.descContains("Search")),5000);
        target.setText("Deep Rock");
        TimeUnit.MILLISECONDS.sleep(1000);
        mDevice.pressKeyCode(66);
        TimeUnit.MILLISECONDS.sleep(1000);

        // Oracle
        UiObject2 element1 = mDevice.findObject(By.textContains("Galactic"));
