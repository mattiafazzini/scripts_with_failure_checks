...
public class new_1234141{
    ...
    public void testChangeText_sameActivity() throws InterruptedException, RemoteException {
        ...
        target = mDevice.wait(Until.findObject(By.text("abc.txt")),2000);
        target.click(2000);
        target.click(2000);
        target = mDevice.wait(Until.findObject(By.desc("More options")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("Hide")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.scrollable(true)),2000);
        target.fling(Direction.UP);
        target = mDevice.wait(Until.findObject(By.desc("More options")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("Hidden Files")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.desc("Delete Bookmark")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("CLOSE")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.scrollable(true)),2000);
        target.fling(Direction.DOWN);
        // Oracle
        UiObject2 element1 = mDevice.findObject(By.text("abc.txt"));
