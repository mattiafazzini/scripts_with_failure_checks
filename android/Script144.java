...
public class Script144 {
    ...
    public void startMainActivityFromHomeScreen()  throws UiObjectNotFoundException {
        ...
        //click
        mDevice.findObject(new UiSelector().className("android.widget.EditText").text("First name")).click();
        mDevice.findObject(new UiSelector().className("android.widget.EditText").text("First name")).setText("john");
        if (isKeyboardOpened()) {
            mDevice.pressBack();
        }
        try {
        ...
        //click
        mDevice.findObject(new UiSelector().className("android.widget.EditText").text("Last name")).click();
        mDevice.findObject(new UiSelector().className("android.widget.EditText").text("Last name")).setText("doe");
        if (isKeyboardOpened()) {
            mDevice.pressBack();
        }
        try {
    ...
    public void performActions() throws UiObjectNotFoundException {
        ...
        //oracle 
        UiObject2 element1 = mDevice.findObject(By.text("jo"));
