...
public class new_123493 {
    ...
    public void testChangeText_sameActivity() throws InterruptedException, IOException, RemoteException {
        ...
        target = mDevice.wait(Until.findObject(By.text("OK")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("real")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.desc("Add")),2000);
        target.click();
        ...
        mDevice.click(100,100);
        TimeUnit.MILLISECONDS.sleep(1000);
        target = mDevice.wait(Until.findObject(By.text("Custom study session")),2000);
        mDevice.swipe(target.getVisibleCenter().x,target.getVisibleCenter().y,target.getVisibleCenter().x,target.getVisibleCenter().y,200);
        target = mDevice.wait(Until.findObject(By.text("Rename deck")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.clazz("android.widget.EditText")),2000);
        target.setText("fake::custom");
        target = mDevice.wait(Until.findObject(By.text("RENAME")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("fake")),2000);
        target.click();
        TimeUnit.MILLISECONDS.sleep(3000);
        target = mDevice.wait(Until.findObject(By.desc("More options")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("Edit note")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.desc("Front")),2000);
        target.setText("edited");
        target = mDevice.wait(Until.findObject(By.desc("Save")),2000);
        target.click();


        //oracle
        UiObject2 element1 = mDevice.findObject(By.text("1"));
