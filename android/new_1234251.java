...
public class new_1234251{
    ...
    public void testChangeText_sameActivity() throws InterruptedException, RemoteException {
        ...
        target.click();
        target = mDevice.wait(Until.findObject(By.res(BASIC_SAMPLE_PACKAGE.concat(":id/txtvTitle"))),2500);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("Stream")),5000);
        if(target == null){
            target = mDevice.wait(Until.findObject(By.textContains("MB")),5000);
            target.click();
            target = mDevice.wait(Until.findObject(By.text("Stream")),5000);
        }
        target.click();
        target = mDevice.wait(Until.findObject(By.text("Pause")),5000);
        target.click();
        mDevice.swipe(540,1675,540,200,20);
        TimeUnit.MILLISECONDS.sleep(4000);
        target = mDevice.wait(Until.findObject(By.desc("More options")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("Open Podcast")),5000);
        target.click();


        // Oracle
        UiObject2 element1 = mDevice.findObject(By.desc("Skip episode"));
