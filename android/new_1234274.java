...
public class new_1234274{
    ...
    public void testChangeText_sameActivity() throws InterruptedException {
        ...
        target = mDevice.wait(Until.findObject(By.desc("Download")),2000);
        target.click();
        mDevice.waitForIdle();
        TimeUnit.MILLISECONDS.sleep(4000);
        target = mDevice.wait(Until.findObject(By.desc("Open menu")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("Episodes")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.desc("More options")),2000);
        target.click();
        TimeUnit.MILLISECONDS.sleep(2000);
        mDevice.click(550,800);
        TimeUnit.MILLISECONDS.sleep(2000);
        target = mDevice.wait(Until.findObject(By.text("ALL")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.desc("More options")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("Filter")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("Downloaded")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("Confirm")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.textContains("MB")),2000);
        target.longClick();
        target = mDevice.wait(Until.findObject(By.text("Delete")),2000);
        target.click();



        // Oracle
        UiObject2 element1 = mDevice.findObject(By.res("de.danoeh.antennapod.debug:id/coverHolder"));
