...
public class new_1234126 {
    ...
    public void testChangeText_sameActivity() throws InterruptedException, RemoteException {
        ...
        target = mDevice.wait(Until.findObject(By.clazz("android.widget.ImageButton").selected(true)),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("Rename")),2000);
        target.click();
        target = mDevice.wait(Until.findObject(By.text("file.txt")),2000);
        target.setText("fi le.txt");
        target = mDevice.wait(Until.findObject(By.text("SAVE")),2000);
        target.click();
        TimeUnit.MILLISECONDS.sleep(2000);

        // Oracle
        UiObject2 element1 = mDevice.findObject(By.text("fi%20le.txt")); 
